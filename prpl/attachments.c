...
static void send_upload_confirmation(PurpleXfer *xfer, const gchar *etag)
{
	...
	chime_connection_send_message_async(
		data->conn,
		data->obj,
		xfer->message,
		NULL,
	...
	gpointer user_data)
{
	...
	if (purple_xfer_is_canceled(xfer)) return deep_free_upload_data(xfer);

	if (!SOUP_STATUS_IS_SUCCESSFUL(msg->status_code)) {
		gchar *error_msg = g_strdup_printf(_("Failed to upload file: (%d) %s"), msg->status_code, msg->reason_phrase);
...
static void put_file(ChimeConnection *cxn, PurpleXfer *xfer)
{
	...
	soup_message_set_request(msg, data->content_type,
			SOUP_MEMORY_TEMPORARY, data->content, data->content_length);
	...
	data->soup_session = soup_session_new_with_options(
			SOUP_SESSION_ADD_FEATURE_BY_TYPE,
			SOUP_TYPE_CONTENT_SNIFFER, // TODO: Check this type
...
static void request_upload_url_callback(
	ChimeConnection *cxn,
	SoupMessage *msg,
	JsonNode *node,
	gpointer user_data
)
{
	...
	if (SOUP_STATUS_IS_SUCCESSFUL(msg->status_code) && node) {
		...
		if (!SOUP_STATUS_IS_SUCCESSFUL(msg->status_code)) {
			...
			gchar *error_msg = g_strdup_printf(_("Failed to request upload: %d %s"), msg->status_code, reason);
			purple_xfer_conversation_write(xfer, error_msg, TRUE);
			g_free(error_msg);
		} else if (!node) {
...
static void request_upload_url(
	ChimeConnection *self,
...
)
{
	...
	JsonNode *node = json_builder_get_root(jb);
	chime_connection_queue_http_request(
			self, node, uri, "POST", request_upload_url_callback, xfer);
...
static void chime_send_init(PurpleXfer *xfer)
{
	...
	if (!g_file_get_contents(xfer->local_filename, &file_contents, &length, &error)) {
		...
		purple_debug_error("chime",
				_("Could not read file '%s' (errno=%d, errstr=%s)\n"),
